...
Evo是一个基于比特币UTXO模型，权益证明机制(pos)和支持EVM智能合约的区块链项目。它通过创新的账户抽象层(Account Abstraction Layer)实现了比特币与以太坊两大生态的融合。如果想了解更多关于Evo的信息，可以访问我们的官网[www.coinevo.tech](www.coinevo.tech)，欢迎加入我们的社区。

## 基于Evo的智能合约
首先需要说明的是，使用Evo的智能合约接口仍然需要一定的技术储备。本文对智能合约的操作在命令行中使用`evo-cli`，或者在图形钱包`evo-qt`的debug窗口中使用。
...
*（提示：如果使用QT钱包中的debug窗口，在以下的命令中都不需要再包含`./evo-cli`）*
...
* Q: "我觉得我发现了一个Evo的bug"
	...
	A: 欢迎报送到这里 [https://github.com/coinevo/evo/issues](https://github.com/coinevo/evo/issues)



# Evo新增的RPC命令

Evo支持所有比特币的RPC命令，并且还新加了如下的命令：

* `createcontract` - 在Evo上创建和发布智能合约，会消耗gas
* `callcontract` - 这是一个与Evo上已经部署好的合约进行交互的接口，所有的计算都是在链下进行，不需要消耗gas
* `sendtocontract` - 这也是一个与Evo上已经部署好的合约进行交互的接口，但是所有的计算都是在链上进行的，并且所有的状态改变都会同步到链上。这个命令可以向合约发送代币，会消耗gas
...
Evo支持所有的比特币命令行参数，另外Evo新增如下的参数：

* `-record-log-opcodes` - 在Evo的数据目录(一般是在~/.evo)下新建一个log文件，名字是`vmExecLogs.json`，所有的EVM LOG操作都会被记录。
